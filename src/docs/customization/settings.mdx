...
<p className="lead">In Spruce CSS, the settings are global options controlled through a Sass map.</p>
...
```
<Notification type="info">We added a simple formula: one value with a relative viewport unit (vw) to make it responsive and one value with a root-relative unit (to set up a minimum value, it never will be smaller than 1rem). You can specify any numerical value, but use quotations if you use an operation.</Notification>
