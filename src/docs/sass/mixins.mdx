...
The `field-icon()` mixin is a getter function for form input backgrounds. Spruce CSS uses custom images (used as [inline SVG](https://github.com/conedevelopment/sprucecss/blob/main/scss/form/_control.scss#L103)) for various inputs (checkbox, radio, select, success, error states).
