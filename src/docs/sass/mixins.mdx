...
Using the `btn-variant-outline` mixin, you can create a new outlined button variant. First, you must declare the colors of the new button (background, foreground is required) in the `$colors` map at the very first `@use` of Spruce.
...
| `$font-weight`   | `400`         | The wieght of the font.                                                                                                              |
...
.form-control:focus {
  @include spruce.focus-ring(
    $type: $form-control-focus-ring-type,
    $border-color: spruce.color(border-focus, form),
    $ring-color: spruce.color(ring-focus, form),
    $box-shadow-type: $form-control-focus-ring-box-shadow-type,
    $ring-width: $form-control-focus-ring-width,
    $ring-offset: $form-control-focus-ring-offset
  );
}
```
