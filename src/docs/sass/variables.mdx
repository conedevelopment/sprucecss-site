...
```
<Notification type="info">
We added a simple formula: one with a relative viewport unit (vw) to make it responsive and one with a root-relative unit (to set up a minimum value, it never will be smaller than 1rem). You can specify any numerical value, but use quotations if you use an operation.
</Notification>
