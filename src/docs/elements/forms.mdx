...
You can set two additional size with: `form-control--sm` and `form-control--lg` modifier class.
...
- The focus state is differs from the button because we can't set box-shadow (it will be cut down).
...
Use the `form-range` class on an `input[type="range"]` to nativly stlye the range input.
...
You can set two additional size with: `form-check--sm` and `form-check--lg` modifier class.
...
You can set two additional size with: `form-switch--sm` and `form-switch--lg` modifier class.
...
- `form-group--stacked`: to stack the inputs beside eachother.
