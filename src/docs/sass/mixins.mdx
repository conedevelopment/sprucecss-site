...
select {
  &:not([multiple]):not([size]) {
    @include spruce.field-icon($select-image, spruce.color(select-foreground, form));
  }
}
...
<Notification type="info">There are scenarios (unfortunately) where it is easier to modify the CSS code than the HTML markup. In our case, we usually style the checkboxes and radios in the <a href="https://hu.wordpress.org/plugins/contact-form-7/">Custom Form 7</a> plugin.</Nofitication>
...
| Name          | Default Value            | Description                                                                         |
|---------------|--------------------------|-------------------------------------------------------------------------------------|
| ```$gutter``` | ```spruce.spacer(m)``` | The margin value between the elements. The value comes from the ```$spacers``` map. |
