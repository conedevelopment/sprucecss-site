...
I think in CSS, determining what’s good is usually opinionated. Of course, there are some absolute truths, but anything in code depends on the context. A lousy code can do a job well; still, we should go for the better. Every project and team has its own needs, so using a universal solution is rarely possible.
...
- A codebase that is easier to maintain. CSS can be chaotic; it is good if you are using a framework, so you are always familiar with each of your projects. It is also good if this framework is well developed and you regularly update it (so your code will be smaller and more modern).
- In my opinion - in CSS context - modern (newer) means better. You clearly can’t compare a 12-based floated grid system with CSS Grid. The last one can do things we hardly could imagine a few years back. Just with the gap (which you can also apply using flexbox) property, we do magic.
- Quality control is always necessary. Using Stylelint, Prefixer, or PurgeCSS, you can set up and follow strict rules to follow. It is helpful for just one person but superior in a team.
...
Writing your CSS properties in alphabetical order can sound a little strange. It was for me, and then I gave it a try.

Declaring the properties based on the alphabet is convenient and straightforward. You don’t have to think about what group with what and in which order. You get a clear rule to follow, and it simplifies things. For some reason, I always tried to group the properties somehow, and it was more work than it deserved.
...
Using the ABC is nothing more than simplification. It is worth giving it a try. With Stylelint, you can quickly check the order (or fix it). For this, we use the Sass Guidelines config https://www.npmjs.com/package/stylelint-config-sass-guidelines. For the implementation, see. https://github.com/conedevelopment/sprucecss/blob/main/.stylelintrc.json
...
A methodology is a lifesaver because we have just a few rules in CSS, which can result in weak code in the long run. Code quality matters. Like our choice, BEM, a methodology can help us to think in components and write more maintainable code.
...
Some excellent CSS architecture:
- [BEM](http://smacss.com/)
...
The following example is the .btn styling from Spruce CSS. For the complete example, check out the source code on GitHub https://github.com/conedevelopment/sprucecss/blob/main/scss/form/_button.scss.
...
I usually write SCSS. I like Sass; it is programmatic CSS which can be handy. It is also a way of processing your code. On a basic level, you can write fewer lines of code, separate them, and merge them as you wish. On a more intermediate level, you can make your small framework. Compiling Sass is easy with sass-cli; I wrote a simple guide about it on Pine.

If your codebase is vast and you don’t control it from the start, you can use Purge CSS to remove the unnecessary lines. For example, if you use the complete bootstrap grid, you will rarely use all of the classes. You can make your CSS 30-50% lighter if you purge it before publishing.

## Go Modern, Real Modern

CSS is evolving fast lately 🥳. We don’t get prefixed properties; the vendors open the support almost simultaneously. If we get a new feature, we can practically use them. Of course, the collective support of the new feature isn’t that easy; always are some exceptions, but we rarely see new stuff with less than 80% on caniuse.com.
...
[aspect-ratio example from spruce css]

If you worry about backward compatibility, make your code progressively enhanced and use @support. And keep in mind that inclusion is still necessary with the new features.

If you need a quick recap about modern CSS, check out the Writing Better CSS article (https://www.aleksandrhovhannisyan.com/blog/writing-better-css/) by Aleksandr Hovhannisyan.
