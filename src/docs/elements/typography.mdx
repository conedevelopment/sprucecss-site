...
To access any of the `$font-sizes` map sizes, you can use the `spruce.font-size()` function that allows you also to create responsive font sizing. The `font-size()` function utilizes the <code><a hhre="https://developer.mozilla.org/en-US/docs/Web/CSS/clamp">clamp()</a></code> CSS function, which gives us the possibility to set a value based on any relative unit (in this case: 5vw + 1rem).
