...
@use '~sprucecss/scss/spruce' with (
  $colors: (
    ...
    btn: (
      ...
      new-foreground-hover: hsl(0, 0%, 10%),
    )
  ),
...
```scss
$settings:(
  color-fallback: false, // Turn on/off the fallback color when using spruce.color() function
  hyphens:  true, // Turn on/off the hyphenation
  optimal-responsive-font-size: '2vw + 1rem' // Optimal size use at font-size() and responsive-font-size() function
);
```
